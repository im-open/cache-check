...
This action will check if an exact match is available in the cache without downloading it.  Caches have unique versions based on the compression tool used and the path of the directories being cached.  As a result, caches generated on Linux machines for instance cannot be restored on Windows machines.  See [actions/cache docs] for more details.

This action is a slimmed down version of the [MartijnHols/actions-cache/check] action.

If you need an action that downloads the cache, checkout the official [actions/cache] action.
...
| --------- | ----------- | ----------------------------------------------------------------------------------------- |
| `paths`   | true        | The list of files, directories and wildcard patterns that were used when saving the cache |
  ...
  setup-caches:
    ...
    steps:
      ...
      - name: Check for an npm cache
        id: has-npm-cache
        uses: im-open/cache-check@v1.0.0
        with:
          paths:  '**/node_modules'
...
[MartijnHols/actions-cache/check]: https://github.com/MartijnHols/actions-cache/blob/main/check/action.yml
